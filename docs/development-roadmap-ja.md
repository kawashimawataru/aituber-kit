...
|--------|-----------|-------------------|
| **VRM** | `[motion:]` + `poseConfigs` | stunt 専用 VRMA/ポーズ、**体傾き用ポーズ**（§3.1）、ルート揺れ、カメラシェイク連動 |
...
現状は §3.1 のとおり **会話連動の傾きはない**。次を stunt カタログに含める。
...
**現状**: §3.1 のとおり **手動・静的** は実装済み。ここでは **会話・感情・配信状況に連動** する拡張のみを扱う（既存 UI は維持）。
...
| 2026-05-23 | §3.1 現状調査（体傾き×・背景手動○）、Phase 6 lean タスク、Phase 6.8 背景ムード連動 |
